...
static inline u64 paravirt_steal_clock(int cpu)
{
	...
	u64 resulter = static_call(pv_steal_clock)(cpu);
	/*if(cpu==0){
		if((resulter-global_steal_previous)>0){
			printk("steal time %llu",resulter);
		}
		global_steal_previous = resulter;
	} */
	return resulter;
}
